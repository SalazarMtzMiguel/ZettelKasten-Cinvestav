...
Sin embargo, replicar la totalidad del ciclo de ingeniería de diseño constituye un desafío multidimensional, con esto referimos al hecho de balancear costos, sostenibilidad, seguridad, resiliencia, etc. \cite{Maier2022} \cite{Crowder2016}. Por ello, y considerando las delimitaciones temporales propias de una investigación de posgrado, este trabajo se acota estrictamente a trabajar con cajas negras y unicamente explicar la actividad del \textit{auto-diseño de algoritmos}. Bajo este alcance, la resolución de dicha actividad se aborda mediante un enfoque formal de asociación problema-solución. Este mecanismo se operativiza a través de una especificación ontológica que proporciona una taxonomía estructurada de problemas, permitiendo al sistema clasificar un requerimiento dinámico y determinar la solución algorítmica idónea para su posterior implementación autónoma.

\section{Planteamiento del Problema}
El diseño de software es una etapa crucial en el ciclo de desarrollo que abarca desde las abstracciones de alto nivel representadas formalmente en las distintas vistas de la arquitectura de software \cite{Kruchten1995The} hasta la especificación granular de sus componentes. Tradicionalmente, esta actividad demanda un consumo elevado de recursos organizacionales y un esfuerzo humano significativo. Aunque las metodologías han evolucionado hacia el paradigma de la Ingeniería de Software 3.0 donde los sistemas basados en agentes autónomos han ganado terreno \cite{li2025riseaiteammatessoftware}, los sistemas actuales carecen de mecanismos para modificar de forma autónoma su lógica algorítmica interna sin alterar o comprometer las estructuras arquitectónicas preexistentes.

Los sistemas no poseen mecanismos de diseño que contemplen un proceso articulado para cada actividad del diseño, sobre todo  descripciones de caja blanca.
...
\item Caracterizar los mecanismos de evolución e implementación autónoma de algoritmos, analizando los criterios técnicos y las restricciones operacionales requeridas para su ejecución sin intervención humana.
...
El Marco Teórico constituye el cimiento conceptual sobre el cual se erige esta investigación. Su propósito fundamental es articular las teorías, paradigmas y modelos previos que otorgan validez científica a la problemática planteada, sirviendo como mapa orientador para el desarrollo de la propuesta.

\section{Sistemas de Auto-Ingenieria}
%\cite perez
En \cite{perez2024} se aborda el hecho de que los mismos sistemas no terminen en desecho si no, que sin intervención humana continúen su operación mediante la evolucionando o en su defecto sufran una metamorfosis dando pie a un nuevo sistema, esta perspectiva se conoce como el marco de los sistemas de auto-ingenieria.
Los SES contemplan un proceso muy similar a la ingenieria de software tradicional, es decir, las actividades como lo son ingenieria de requerimientos, diseño, implementación, pruebas, etc.\cite{sommerville2011software} tienen lugar en este marco de trabajo. Para este trabajo unicamente nos enfocamos en el diseño del software \cite{swebok2014}, esto quiere decir que nos vamos a dedicar a entregar artefactos de diseño, nuestros planos del sistema antes de implementar en código.

La teoría SES introduce conceptos que permiten identificar fases y actividades dentro de la vida de estos sistemas \cite{PerezCarrillo2026}:

\subsubsection{Fases de Operación y Mantenimiento (O \& M) y Evolución}
A diferencia de la literatura tradicional que debate si el mantenimiento es un subconjunto de la evolución o viceversa, el marco SES establece las fases de Operación y Mantenimiento (O\&M) y Evolución como fases independientes.
\begin{itemize}
	\item Fase de O\&M: Se ejecutan mantenimientos correctivos, preventivos, adaptativos, aditivos y de mejora.
	
	\item 	Evolución por Mantenimiento: Se genera cuando los mantenimientos adaptativos, aditivos o perfectivos provocan cambios en los requerimientos.
	\item Evolución por Cambio de Problema: Ocurre cuando cambia el problema que el sistema busca resolver, siempre y cuando la arquitectura actual sea capaz de soportar dichos cambios.
\end{itemize}
\subsubsection{Actividades de Self-O \& M y Self-Evolution}
Son las actividades fundamentales que el sistema ejecuta de manera autónoma tras su despliegue dentro de las fases de O\&M y Evolución, respectivamente.
\subsubsection{Punto de Ruptura / Breaking Point}
Basado en la analogía de los puntos de inflexión (tipping points) en sistemas complejos, representa el umbral en el cual la arquitectura original del sistema ya no puede satisfacer los nuevos requerimientos.
Al alcanzar el \textbf{Breaking Point}, se suspenden las actividades de Self-O\&M y Self-Evolution, desencadenando el inicio de un nuevo proceso e inicio de ciclo de vida de ingeniería.
\subsubsection{Elasticidad del Sistema / System Elasticity}
Fundamentada en los conceptos de evolucionabilidad (evolvability) y resiliencia.
Se define como la capacidad del sistema para asimilar requerimientos nuevos o modificados sin la necesidad de construir un nuevo sistema.
\subsubsection{Metamorfosis / Metamorphosis}

Es la fase fundamental del paradigma SES, definida como la \textbf{trascendencia del sistema original (Sistema A) hacia un nuevo sistema (Sistema B)} mediante un nuevo proceso de Auto-Ingeniería tras alcanzar un punto de ruptura.
Involucra la ejecución coordinada de las actividades SES \textit{Self-*: *Self-Requirements, Self-Design, Self-Implementation, Self-Testing* y *Self-Deployment.}

Se desencadena bajo dos condiciones principales:
\begin{itemize}
	\item  Cuando se requiere un cambio de arquitectura a nivel conceptual (el problema o dominio de aplicación cambió y la arquitectura actual no puede resolverlo).
	\item Cuando los cambios de requerimientos impactan la arquitectura lógica y esta es incapaz de acomodarlos.
\end{itemize}
A diferencia de la ingeniería tradicional donde un arquitecto humano toma la decisión, en el marco SES el propio sistema toma la decisión de manera autónoma.

\subsubsection{Gestión de Cambios de Requerimientos / Requirements Change Management (RCM)}


Estructura de requerimientos funcionales: Se dividen en dos componentes principales: \textbf{Funcionalidad} (las acciones principales expresadas en verbos) y \textbf{Configuración} (parámetros de tiempo, modo o condición). En los requerimientos no funcionales, las métricas y parámetros se tratan como componentes de configuración.
Impacto según el componente:
Si el cambio ocurre solo en la configuración, el sistema lo absorbe dentro de sus fases de O\&M y evolución.
Si el cambio afecta la funcionalidad o genera un impacto en la arquitectura que la actual no soporta, se provoca una metamorfosis.
Proceso RCM adaptado para SES (basado en \cite{sommerville2011software}):
\begin{itemize}
	\item  \textit{Especificación del Cambio}: Recepción y validación de la solicitud de cambio.
	\item  \textit{Análisis del Cambio}: Evaluación de la elasticidad del sistema para determinar si el cambio produce una nueva versión o si se alcanzó el Breaking Point.
	\item  \textit{Implementación del Cambio}: Si la arquitectura lo soporta, el sistema evoluciona a una nueva versión; si no lo soporta, se inicia un nuevo proceso de Auto-Ingeniería dando lugar a un nuevo sistema (metamorfosis).
\end{itemize}


\subsection{Meta-Arquitectura del Marco de Sistemas de Auto-Ingeniería}

Los SES cuentan con el ether, el centro de operacion de estos sistemas, recordando el hecho de la existencia de dos momentos clave, la evolucion y la metamorfosis, Un sistema SES busca gestionar y ejecutar su propio proceso de ingenieria mas alla de su ciclo de vida convencional, prescindiendo de la intervencion humana. el ether se encarga de primeramente el mantenimiento el cual escorregir errores y defectos, 

Los SES manejan la evolucion como una fase independiente a la de operacion y mantenimiento en el ciclo de vida del software, los mantenimientos correctivos, preventivos, adaptativos, aditivos y de mejora \cite{ISOIEEE} pueden dar lugar a la fase de evolucion.
...
\begin{enumerate}
	...
	\item Diseño de Estructuras de datos
...
Esta especificacion segun \cite{sommerville2011software} es la parte en la cual se hace una vision general del sistema y como de be operar.

\subsection{Diseño de Interfaces}
Esta actividad especifica como es que las interfaces van a aoperar antes de incluso diseñar componentes, una vez declarado esto los componentes solo adaptas estas maneras de comunicacion para garantizar cohesion y acoplamiento requerido.



\subsection{Diseño de Componentes}

Aqui la actividad especifica como cada componente opera, cumpliendo funciones y objetivos especificos.

\subsection{Diseño de Estructuras de datos}
Esta parte funciona para modelar todas la estructuras de datos que se usaran en los algoritmos, ademas de proporcionar diferentes esquemas para las bases de datos y viceversa, es decir se pueden moldear tablas a partir de estructura de datos necesarias o diseñar estructuras de datos a partir de los requerimientos de información en las bases de datos.


\subsection{Diseño de Algoritmos}
Esta es la actividad en la que indagaremos, se encarga del diseño de algortimos, segun la necesidad, eficiencia y eficacia de lo que necesitemos resolver.


%\section{Diseño Automatizado de Algoritmos}

\section{Sistemas multiagentes MAS}
Los sistemas multiagentes abordan un paradigma donde cada agente puede ser de distinto tipom, reactivos, BDI, basados en logica, basados en planeacion, basados en utilidad, basados en aprendizajes, de pizarra negra, basados en mercado, o hibridos mezclando caracteristicas entre los mismos.
\subsection{Fundamentos y arquitecturas de los Sistemas Multiagentes}
Un agente se puede caracterizar por cualquier elemento que tenga una funcion.
\subsection{Tipología cognitiva de agentes: Modelos de caja negra (puramente estadísticos/reactivos	) vs. Modelos de caja gris (híbridos o neuro-simbólicos)}

\subsection{Paradigmas de coordinación y comunicación inter-agente}

\section{Modelos grande de lenguaje (LLM)}
Los LLM son tecnologias que procesan el lenguaje natural, derivado de un entrenamiento previo del modelo, estas son grander herramientas que por si solas ayudan mucho a tareas de escritura y de analisis derivado de los patrones que ya reconocen.
\subsection{Principios de inferencia y generación de texto en modelos de lenguaje}
Los Modelos son capaces de inferir cierta informacion basado en el contexto dado en los prompts, y su entrenamiento, por eso, puede responder segun reglas de escritura a lo que se ponga de entrada, dado esto, funcionan como una caja negra, tenemos una entrada un prompt y la salida de dicho prompt, la respuesta varia de modelo a modelo, por el entrenamiento, la ventana de contexto y la calidad de la informacion.
\subsection{Capacidades de los LLMs en la comprensión, diseño y generación de código}
Como tal el contexto dado al LLM rige que puede dar como salida, en el caso de la ingenieria de software, el modelo debe de estar entranado con mucha informacion formal y de campo del area de dominio, para de esa manera reconocer patrones y replicarlos dado el input de la funcion, en el caso de la ingenieria de diseño, dar la especificacion de los patrones de diseño, y poder compara con su propio entrenamiento que plantillas se ajustan mas a lo requerido, es decir, si nos piden una web sencilla con interacciones, quiza lo mejor sea un comprtamiento monolitico, pero bajo el entrenamiento del LLM nos recomiende mejor un enfoque de MVC (Modelo, Vista, Controlador).
\subsection{Limitaciones actuales: Alucinaciones, ventanas de contexto y deficiencias en el razonamiento algorítmico determinista}
Existen varios contras de los LLM, el primero con las alucinaciones, el segundo es la falta de contexto,  esto puedo derivar en malas respuestas, conocidas como alucinaciones, supongamos que si o si los LLM generan una respuesta entonces retornan lo mas "cercano" a la respuesta correcta segun su entrenamiento y el contexto dado, por eso aveces aunque no sea una respuesta correcta, contestan algo aproximado y aveces si es que su modelo no tiene informacion relevante, si divaga y contesta algo que no tiene nada en comun, eso es una alucinacion, ahora bien en ventanas de contexto, tenemos 2 tipos, de entrenamiento y de tokens, los tokens son los valores dados a ciertas cadenas de texto, estas nos permiten dar importancia a lo escrito, por ejemplo la mujer tiene el pelo verde, con tres palabras relevantes podemos sacar lo mas valioso del la frase, mujer, pelo y verde, con eso sabemos que es lo que queremos comunicar.

\section{Arbol de busqueda de Monte-Carlo (MCTS)}
Un Monte Carlo Tree Search, (MCTS) es un algoritmo de búsqueda y toma de decisiones que construye un árbol de forma incremental usando simulaciones aleatorias para evaluar opciones y elegir la acción más prometedora, cada nodo es un estado o configuraciones de un problema y las aristas son transiciones (acciones) de nodo a nodo \cite{mctswiechowski2022}.

Suele repetirse en cuatro fases: selección, expansión, simulación y propagacion hacia atras (retropropagacion). 

\begin{itemize}

\item \textbf{Selección}: se baja por el árbol eligiendo nodos “buenos” según una regla como UCB/UCT, que equilibra exploración y explotación. 

\item \textbf{Expansión}: se añade uno o más nodos hijos para representar nuevas acciones posibles. 

\item \textbf{Simulación}: se hacen jugadas o recorridos aleatorios (o guiados) desde ese nodo para estimar su valor. 

\item \textbf{Retropropagacion}: el resultado de la simulación se propaga hacia arriba para actualizar visitas y valores de los nodos anteriores.

\end{itemize}

\subsection{Espacios de búsqueda y métodos de toma de decisiones secuenciales}

\subsection{Fases algorítmicas del MCTS}
El arbol de busqueda de montecarlo nos permite identificar patrones en un conjunto de soluciones, nos permite converger en una entre mas iteraciones tengamos.

\section{Ontología como representacion de conocimiento}

Una ontología es una especificación formal y explícita de un dominio: define conceptos, relaciones, individuos, axiomas y restricciones para que el conocimiento pueda ser compartido, reutilizado y procesado por máquinas \cite{OntologyGuarino2009}.
Se caracterizan por representaciones semánticas porque no solo organiza datos, sino que hace explícito el significado de esos datos y las relaciones entre entidades, lo que permite inferencia, interoperabilidad y comprensión automática por software.

\subsection{Definición y estructura de las ontologías computacionales (clases, atributos, relaciones)}


\subsection{Lenguajes de modelado ontológico}
Aqui va OWL

\subsection{Modelado semántico para la taxonomía y clasificación de problemas algorítmicos}
El trabajo de \cite{duran2026seleccion} une estos temas, para lograr la seleccion automatica de algoritmos, esto en sistemas del internet de las cosas (IoT), la ontologia deriva del trabajo \cite{DURANPOLANCO2023100904} donde se propone una taxonomia para la toma de decisiones bajo incertidumbre, considera caracteristicas como el Objetivo, Tipo de Procesamineto, Capa de procesamiento, Interaccion, incertidumbre, representacion de conocimiento y un enfoque algoritmico.
...
En la basta literatura encontramos numerosos diversos enfoques para cubrir parcialmente o totalmente lo que se considera como auto-diseño.
...
Bajo un enfoque de agentes de IA \cite{Dam2025TowardsMulti-Agentic}, introduce un marco de trabajo para modelar y diseñar software, consta de 
El trabajo de \cite{Maadli2025} propone un 

\cite{Rajendran2025AMulti-Agent}
\cite{Affonso2024AReferenceArchitecture}
\cite{Houichime2024Optimized}
\cite{Zhao2025AutomatedMetaheuristic}
\cite{AuroraRamirez2015234}
\cite{WeynsDannyFromSelf-Adaptation}
...
El auto-diseño se define como el conjunto de tareas que un sistema realiza para dar como resultado artefactos de diseño, especificaciones claras y un documento que cubra íntegramente lo que en el futuro sera el software especificado por los requerimientos, ya sea en caso de elasticidad (operación y mantenimiento o evolución) o de metamorfosis.


\subsection{Desarrollo de la metodologia Prometheus}

Para dar solucion a este problema, se empleó la metodologia de prometheus, para poder diseñar y brindar un sistema multiagente con un flujo de trabajo determinado y una descripcion de caja blanaca del agente de algoritmos, evidentemente  lo ideal es que todos los agentes sean de caja blanca pero esta fuera del alcance de este trabajo. \cite{prometheus_padgham2005}

Prometheus cubre tres fases:

\begin{itemize}
	\item 1. La fase de especificación del sistema se centra en identificar los objetivos y las funcionalidades básicas del sistema, junto con las entradas (percepciones) y las salidas (acciones).
	\item 2. La fase de diseño arquitectónico utiliza los resultados de la fase anterior para determinar qué tipos de agentes incluirá el sistema y cómo interactuarán entre sí.
	\item 3. La fase de diseño detallado analiza el funcionamiento interno de cada agente y cómo llevará a cabo sus tareas dentro del sistema en general.
\end{itemize}
\subsubsection{Especificacion del sistema}

\subsubsection{Diseño Arquitectonico}
...
El sistema se estructura como un sistema multiagente orientado al proceso de diseño de software. Los agentes colaboran mediante un flujo de trabajo secuencial, en el cual cada agente posee una responsabilidad de diseño específica y produce información que condiciona las decisiones de los agentes posteriores. La metodología Prometheus se emplea para identificar y modelar los agentes, sus metas, percepciones, acciones, capacidades e interacciones.
